...
As funções são parte das ferramentas que podemos usar para modularizar o código, deixando ele mais legível, organizado e facilitando que mais de uma pessoa trabalhe no código.
...
- **argv** (vetor de argumentos) é um vetor de ponteiros para char que armazena os argumentos de linha de comando na forma de strings. Cada string é um argumento separado e é representado como uma matriz de caracteres terminados por nulo.
...
0 O que são parâmetros de uma função?
